...
@app.get("/")
def inicio():
  return {"status": "online", "sistema": "Faturamento Médico API"}
...
async def processar_notas(arquivos: list[UploadFile] = File(...)):
  dados_extraidos = []
  caminhos_temp = []
  ...
  try:
    ...
    for arquivo in arquivos:
      ...
      caminhos_temp.append(arquivo_temp.name)

    # ====================================================
    # PROCESSAR TODOS OS ARQUIVOS
    # ====================================================

    notas = ler_arquivos(caminhos_temp)
  ...
  return {"quantidade": len(dados_extraidos), "dados": dados_extraidos}
...
async def calcular_faturamento(dados: dict):
  dados_extraidos = dados.get("dados", [])

  honorario_total = float(dados.get("honorario_total", 0))

  darf_gps_total = float(dados.get("darf_gps_total", 0))
  ...
  medicos = sorted(set(nota.get("Medico", "") for nota in dados_extraidos))
  ...
  if quantidade_medicos > 0:
    honorario_por_medico = honorario_total / quantidade_medicos
    darf_gps_por_medico = darf_gps_total / quantidade_medicos
  else:
  ...
  demonstrativo, totais_a_pagar = gerar_demonstrativo(
      dados_extraidos,
  ...
  protocolo = gerar_protocolo(
      ...
      total_faturamento=totais["total_faturamento"],
      total_iss=totais["total_iss"],
      total_inss=totais["total_inss"],  
  ...
  demonstrativo_json = demonstrativo.fillna("").to_dict(orient="records")

  # ========================================================
  # CONVERTER PROTOCOLO PARA JSON
  # ========================================================

  protocolo_json = protocolo.fillna("").to_dict(orient="records")
...
async def gerar_excel(dados: dict):
  ...
  honorario_por_medico = float(dados.get("honorario_por_medico", 0))

  darf_gps_por_medico = float(dados.get("darf_gps_por_medico", 0))
  ...
  if not dados_extraidos:
    return {"erro": "Nenhuma nota fiscal foi informada."}
