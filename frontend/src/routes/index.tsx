...
function HomePage() {
  ...
  // TOTAIS DAS NOTAS
  // ============================================================
  const faturamento = calculos?.totais?.total_faturamento || 0
  const iss = calculos?.totais?.total_iss || 0
  const inss = calculos?.totais?.total_inss || 0
  const pis = calculos?.totais?.total_pis || 0
  const cofins = calculos?.totais?.total_cofins || 0
  const csll = calculos?.totais?.total_csll || 0
  const ir = calculos?.totais?.total_ir || 0
    ...
    try {
      const resposta = await fetch(
        'http://127.0.0.1:8000/faturamento/excel',
        {
          ...
          body: JSON.stringify({
            dados: notas,
            honorario_por_medico: calculos?.honorario_por_medico || 0,
            darf_gps_por_medico: calculos?.darf_gps_por_medico || 0,
          }),
        }
      )
  ...
  const totalProtocolo = Number(
    protocolo.find((item: any) => item['Descrição'] === 'TOTAL')?.['Valor R$'] || 0
  )
    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <main className="mx-auto max-w-7xl space-y-8 px-6 py-8">
        ...
        <div className="flex items-center gap-2 text-sm">
          ...
          <span className="font-semibold text-slate-700">Faturamento</span>
        </div>
        ...
        <UploadPanel
          onNotasProcessadas={(dados) => {
            setNotas(dados)
            calcularFaturamento(dados, honorarioTotal, darfGpsTotal)
          }}
        />
        ...
        <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
          <div className="mb-5">
            <h2 className="text-xl font-semibold text-slate-900">Gerar Excel</h2>
        ...
        {notas.length > 0 && (
          <>
                    ...
                    <tbody>
                      {notas.map((nota, index) => (
                        ...
                        >
                          ...
                          <td className="whitespace-nowrap px-5 py-3 text-right text-slate-600">
                            {formatarReais(
                              (nota.ValorPis || 0) + (nota.ValorCofins || 0)
                            )}
                          </td>
                          <td className="whitespace-nowrap px-5 py-3 text-right text-slate-600">
            ...
            <section>
              ...
              <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                ...
                <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                  <p className="text-sm font-medium text-slate-500">ISS</p>
                ...
                <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                  <p className="text-sm font-medium text-slate-500">INSS</p>
                  <p className="mt-2 text-2xl font-bold text-slate-900">
                    {formatarReais(inss)}
                  </p>
                </div>
                ...
                <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                  <p className="text-sm font-medium text-slate-500">PIS</p>
                  <p className="mt-2 text-2xl font-bold text-slate-900">
                    {formatarReais(pis)}
                  </p>
                </div>
                ...
                <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                  <p className="text-sm font-medium text-slate-500">COFINS</p>
                  <p className="mt-2 text-2xl font-bold text-slate-900">
                ...
                <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                  <p className="text-sm font-medium text-slate-500">CSLL</p>
                  <p className="mt-2 text-2xl font-bold text-slate-900">
                    {formatarReais(csll)}
                  </p>
                </div>
                ...
                <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                  <p className="text-sm font-medium text-slate-500">IR</p>
                  <p className="mt-2 text-2xl font-bold text-slate-900">
                    {formatarReais(ir)}
                  </p>
                </div>
            ...
            <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(event) => {
                      ...
                      calcularFaturamento(notas, valor, darfGpsTotal)
                    }}
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(event) => {
                      ...
                      calcularFaturamento(notas, honorarioTotal, valor)
                    }}
                    className="w-full rounded-xl border border-slate-300 px-4 py-3 text-sm outline-none transition focus:border-blue-500 focus:ring-2 focus:ring-blue-100"
                  />
                </div>
              ...
              {calculando && (
                <p className="mt-4 text-sm text-blue-600">Calculando...</p>
              )}
                  ...
                  <table className="w-full min-w-[700px] text-left text-sm">
                    ...
                    <tbody>
                      {(calculos?.demonstrativo || [])
                        .filter((linha: any) => linha['Médico'] !== 'TOTAL')
                        .map((linha: any, index: number) => (
                          <tr key={index} className="border-b border-slate-100">
                            ...
                            <td className="px-6 py-4 text-right font-semibold text-slate-800">
                              {formatarReais(
                                Number(linha['Valor dos Impostos'] || 0)
                              )}
                            </td>
                              ...
                              <button
                                ...
                                onClick={() => {
                                  ...
                                  sessionStorage.setItem(
                                    ...
                                    String(calculos?.honorario_por_medico || 0)
                                  )
                                  sessionStorage.setItem(
                                    'darf_gps_por_medico',
                                    String(calculos?.darf_gps_por_medico || 0)
                                  )
                                  navigate({
                                    to: '/medico',
                                    search: {
              ...
              <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
                <div className="overflow-x-auto">
                  <table className="w-full min-w-[700px] text-left text-sm">
                    ...
                    <tbody>
                      {protocolo
                        .filter((item: any) => item['Descrição'] !== 'TOTAL')
                        .map((item: any, index: number) => (
                          <tr key={index} className="border-b border-slate-100">
                            ...
                            <td className="px-6 py-4 text-right font-medium text-slate-800">
                              {formatarReais(Number(item['Valor R$'] || 0))}
                            </td>
                          </tr>
                        ))}
